...
const checker=(itemsArray,primitiveValue) =>(
    {
        exists : itemsArray.some(item => item === primitiveValue),
        index : itemsArray.findIndex(item => item === primitiveValue),
        allElementsAreNumbers : itemsArray.every(item => Number(item)===item),
        someElementsAreNumbers : itemsArray.some(item => Number(item) === item)
    }
);
